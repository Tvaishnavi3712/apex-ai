...
# backend/services/aws_provisioner.py) seeds Cosmos DB with these exact
...
def _write_prereqs(doc: Document):
    ...
    _add_bulleted(doc, [
        ...
        "Cosmos DB tables provisioned. Run: "
...
def _write_testing_methodology(doc: Document):
    ...
    _add_bulleted(doc, [
        ...
        "RUNTIME errors that reference a missing Cosmos DB table indicate the provisioner hasn't "
...
def _write_troubleshooting(doc: Document):
    ...
    _add_para(doc, "The handler hit Cosmos DB but the table doesn't exist yet. Run the provisioner:")
