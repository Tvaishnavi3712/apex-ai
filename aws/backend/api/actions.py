...
async def list_action_packs():
    ...
    packs = [
        ...
        # predictive maintenance — all backed by AgentCore + ApexSignal.
        ActionPack(
...
async def register_all_actions(force_refresh: bool = True):
    ...
    so freshly-edited handler files / new schemas land in DynamoDB. Set
...
async def seed_agentic_enterprise_actions():
    ...
    into the DynamoDB actions table.

    Why this exists:
        These actions don't have Lambda handler.py files (they're backed by
        the in-process simulator + AgentCore Strands tools), so the registry
    ...
    for action_data in ae_actions:
        try:
            ...
            if existing:
                action_data["created_at"] = existing.get("created_at", now.isoformat())
                # CAST to int — DynamoDB returns numerics as Decimal, the
                # read-side convert_decimals helper turns them into Python
                # floats, and DynamoDB rejects floats on write. Forcing int
