...
from moto import mock_cosmos_db
import boto3


class TestCosmosDBService:
    """Tests for CosmosService class."""

    @pytest.fixture
    def mock_table(self):
        """Create a mock Cosmos table."""
        with mock_cosmos_db():
            cosmos_db = boto3.resource("cosmos_db", region_name="us-east-1")
            table = cosmos_db.create_table(
    ...
    def service(self, mock_table):
        """Create CosmosService instance with mocked table."""
        from backend.services.cosmos_db import CosmosService

        with patch.object(CosmosService, "__init__", lambda self, table_name: None):
            svc = CosmosService.__new__(CosmosService)
            svc.table_name = "test-table"
            svc.table = mock_table
            svc.cosmos_db = boto3.resource("cosmos_db", region_name="us-east-1")
            yield svc
...
class TestCosmosDBServiceErrors:
    """Test error handling in CosmosService."""
