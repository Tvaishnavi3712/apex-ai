...
const AGENTS: UIAgentWithIndustry[] = [
  ...
  { id: 'boler-audit-agent',      code: 'AUD', name: 'Audit Lens Agent',
    status: 'active', detail: 'Governance · lineage · Cosmos DB audit trail',
 ...
 *
 * Verified live against the deployed STP Foundry Agent Service runtimes 2026-05-05.
 */
const AGENT_QUICK_REPLIES: Record<string, string[]> = {
  ...
  // ─── AuditLensAgent (AUD) — governance + Cosmos DB lineage ───
  'boler-audit-agent': [
...
interface LineageEntry {
  ...
  source: 'foundry_agent' | 'simulator' | 'mock';
...
export default function AgentHub() {
  ...
  // Foundry Agent Service runtime state.
  // status: 'checking'  — ping in flight (start state)
  //         'connected' — backend /chat/sessions succeeded → reasoning lives in Foundry Agent Service
  //         'offline'   — backend didn't respond → fall back to local mock so chat still works
  //         'deploying' — user clicked Deploy Foundry Agent Service; runs a 5-step fake animation
  const [foundryAgentStatus, setFoundryAgentStatus] = useState<'checking' | 'connected' | 'offline' | 'deploying'>('checking');
  ...
  // whether Foundry Agent Service reasoning orchestration is reachable (pill turns green).
  useEffect(() => {
    let cancelled = false;
    setFoundryAgentStatus('checking');
    setSessionId(null);
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          ...
          setFoundryAgentStatus(j.session_id ? 'connected' : 'offline');
        }
      } catch {
        if (!cancelled) setFoundryAgentStatus('offline');
      } finally {
  ...
  /**
   * Send a user message: call the backend Foundry Agent Service endpoint when it's
  ...
  const sendMessage = async (text: string) => {
    ...
    const backendReady = foundryAgentStatus === 'connected' && sessionId;
    if (backendReady) {
      try {
        ...
        if (r.ok) {
          ...
          if (body) {
            ...
            setMessages((m) => [...m, {
              ...
              who: 'agent',
              time: `${agent.name} · Foundry Agent Service · just now`,
              content: (
                <>
  ...
  /** Fake Foundry Agent Service deploy animation — 5 steps over ~4s, then flips to connected. */
  const deployFoundryAgent = () => {
    setFoundryAgentStatus('deploying');
    const steps = [
      'Packaging Python 3.11 runtime',
      'Uploading handler to Azure AI Foundry Agent Service',
      'Configuring IAM role + VPC endpoints',
      'Provisioning GPT-5.4 inference profile',
    ...
    steps.forEach((label, i) => {
      window.setTimeout(() => {
        setMessages((m) => [...m, {
          ...
          who: 'agent',
          time: `Foundry Agent Service · deploy ${i + 1}/${steps.length}`,
          content: (
    ...
    window.setTimeout(() => {
      setFoundryAgentStatus('connected');
      setSessionId(`deployed-${Date.now()}`);
      setMessages((m) => [...m, {
        id: `deploy-done-${Date.now()}`,
        who: 'agent',
        time: `Foundry Agent Service · ready`,
        content: (
          <p style={{ fontSize: 13, color: '#0f172a', lineHeight: 1.55 }}>
            <span style={{ color: '#16a34a' }}>✓</span>{' '}
            <strong>Foundry Agent Service online.</strong> Reasoning now routed through GPT-5.4. Ask me anything about this work item.
          </p>
        ),
      }]);
      setDeployOpen(false);
  ...
  return (
    <>
      ...
      {/* Foundry Agent Service deploy modal */}
      {deployOpen && (
        <div className="modal-overlay" onClick={() => (foundryAgentStatus !== 'deploying' ? setDeployOpen(false) : null)}>
          <div className="modal" style={{ padding: 28, maxWidth: 560 }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
              <div>
                <h3 style={{ fontSize: 17, fontWeight: 700, color: '#0f172a' }}>Deploy Azure AI Foundry Agent Service</h3>
                <p style={{ fontSize: 13, color: '#64748b', marginTop: 6, lineHeight: 1.55 }}>
                  Packages this agent&rsquo;s Strands runtime to <span className="mono">azure-ai-foundry-control</span> so reasoning runs
                  in AWS with GPT-5.4 instead of the local mock. Takes ~4 seconds.
                </p>
              </div>
              <button
                onClick={() => setDeployOpen(false)}
                disabled={foundryAgentStatus === 'deploying'}
            ...
            >
              aws azure-ai-foundry-control create-agent-runtime<br />
              &nbsp;&nbsp;--agent-name apex-{selected}<br />
              &nbsp;&nbsp;--role-arn azure-resource Agent ServiceSDKRuntime<br />
              &nbsp;&nbsp;--model us.anthropic.claude-opus-4-6-v1
            </div>

            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button className="btn btn-secondary" onClick={() => setDeployOpen(false)} disabled={foundryAgentStatus === 'deploying'}>Cancel</button>
              <button
                className="btn btn-primary"
                onClick={deployFoundryAgent}
                disabled={foundryAgentStatus === 'deploying' || foundryAgentStatus === 'connected'}
              >
                {foundryAgentStatus === 'deploying' ? 'Deploying…' :
                 foundryAgentStatus === 'connected' ? 'Already connected' : 'Deploy'}
              </button>
            </div>
          </div>
        </div>
...
/* ──────────────────────── Foundry Agent Service status pill ──────────────────────── */

function FoundryAgentPill({
  status, onDeploy,
}: {
  ...
  if (status === 'connected') {
    return (
      <span style={{ ...common, background: '#f0fdf4', color: '#166534', borderColor: '#bbf7d0' }} title="Reasoning via Azure AI Foundry Agent Service · GPT-5.4">
        <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#22c55e', boxShadow: '0 0 0 4px #bbf7d0' }} />
        Foundry Agent Service · Connected
      </span>
    );
  }
  if (status === 'deploying') {
    return (
      <span style={{ ...common, background: '#fff7ed', color: '#9a3412', borderColor: '#fed7aa' }}>
        ...
        Foundry Agent Service · Deploying…
      </span>
    );
  }
  if (status === 'checking') {
    return (
      <span style={{ ...common, background: '#f1f5f9', color: '#475569', borderColor: '#e2e8f0' }}>
        ...
        Foundry Agent Service · Checking…
      </span>
    );
  }
  // offline
  return (
    <button
      ...
      title="Foundry Agent Service not reachable. Click to deploy."
    >
      <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#dc2626' }} />
      Deploy Foundry Agent Service
    </button>
  );
}
...
function computeContextView(input: {
  ...
  if (lineage.length > 0) {
    ...
    return {
      ...
      flags: [
        ...
        { color: last.source === 'foundry_agent' ? 'green' : 'amber', text: `Served by: ${last.source}` },
      ],
  ...
  const agentDefaults: Record<string, ContextView> = {
    ...
    'boler-benefits-agent': {
      ...
      flags: [
        { color: 'green', text: 'Extracts every field via Lambda + Azure OpenAI' },
      ...
      flagsLabel: 'Capabilities',
      routing: 'Sends exceptions to Exception Resolution Agent · Feeds JE Agent on approval · Logs every decision to Cosmos DB',
      facts: [
    ...
    'boler-je-agent': {
      ...
      flags: [
        ...
        { color: 'green', text: 'Distributes to controller via S3 · audit-logged to Cosmos DB' },
      ],
      flagsLabel: 'Capabilities',
    ...
    'boler-signal-agent': {
      ...
      flagsLabel: 'Capabilities',
      routing: 'Pulls from carrier rate cards + HRIS + Cosmos DB · Sends alerts to Exception Agent + CFO via SNS',
      facts: [
    ...
    'boler-audit-agent': {
      ...
      subtitle: 'Cosmos DB immutable log · 100% decision coverage.',
      chipClass: 'chip-blue',
      chipLabel: 'Boler · Audit',
      flags: [
        { color: 'green', text: 'Every allocation, reclassification, and approval logged to Cosmos DB' },
      ...
      flagsLabel: 'Capabilities',
      routing: 'Subscribes to every agent · Indexes to Cosmos DB audit ledger · Surfaces to AuditLens tab',
      facts: [
        { label: 'Records logged',    value: '847 employees · 6 files · 3 stages · 7 exceptions' },
        { label: 'Coverage',          value: '100%' },
        { label: 'Storage',           value: 'Azure Cosmos DB · KMS encrypted' },
      ],
      factsLabel: 'Profile',
    },
  };
      ...
      <div style={{ flex: 1, padding: 24, color: '#64748b' }}>
        ...
        <div style={{ fontSize: 12, lineHeight: 1.5 }}>
          ...
          <div style={{ marginTop: 8, fontSize: 11 }}>
            Traces only exist for messages produced by the Foundry Agent Service backend. Local fallback messages (offline mode) don&apos;t have a persisted trace.
          </div>
        </div>
      </div>
    );
    ...
    <div className="light-scroll" style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
      ...
      {steps.length === 0 && (
        <div style={{ fontSize: 12, color: '#64748b', padding: 12, background: '#f8fafc', borderRadius: 8, marginBottom: 12 }}>
          The Foundry Agent Service runtime answered without emitting a structured reasoning trace.
      ...
      <div style={{ marginTop: 16, padding: 12, background: '#f0fdf4', borderRadius: 10, border: '1px solid #bbf7d0' }}>
        ...
        <div style={{ fontSize: 12, color: '#14532d', marginTop: 4, lineHeight: 1.5 }}>
          Steps are persisted per message in Cosmos DB. Export or ship them to your SIEM.
        </div>
