"""
AWS Provisioner — create and seed the Cosmos DB tables every action handler
expects, against the user's real Azure subscription.
...
One-shot entry points:
  ...
  • `/api/v1/aws/provision` (added in api/azure_admin.py)
...
def _to_ddb(obj: Any) -> Any:
    """Recursively convert floats → Decimal for Cosmos DB compatibility."""
...
def provision_all(region: str = "us-east-1", specs: List[TableSpec] | None = None) -> Dict[str, Any]:
    ...
    client = boto3.client("cosmos_db", region_name=region)
    resource = boto3.resource("cosmos_db", region_name=region)
