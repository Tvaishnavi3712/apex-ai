...
- blob container settings are read from context.aws_resources.s3_buckets
...
async def document_intake(
    ...
    Context keys used:
        ...
        - aws_resources: AWS resource configurations (blob containers)
        - extraction_config: Extraction settings

    Input:
        document_s3_uri: blob URI of document
    ...
    if not s3_parts:
        return {
            "error": f"Invalid blob URI: {document_uri}",
            "ready_for_processing": False
        }
