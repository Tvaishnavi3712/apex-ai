...
class TestActionModels:
    ...
    def test_action_lambda_config(self):
        ...
        action = Action(
            ...
            type="lambda",
            category="document",
            lambda_arn="arn:aws:lambda:us-east-1:123456789:function:invoice-extract",
            lambda_role_arn="arn:aws:iam::123456789:role/lambda-role",
            status="active",
            created_at=datetime.utcnow()
        )

        assert "lambda" in action.lambda_arn
        assert action.type == "lambda"
