"""
AWS admin endpoints — provision the DynamoDB tables action handlers expect,
...
from services.aws_provisioner import provision_all, TABLE_SPECS
...
async def provision_tables(region: str = Query("us-east-1")) -> Dict[str, Any]:
    """
    Create + seed every DynamoDB table that action handlers expect.
