...
async def _store_exception(exception_record: Dict[str, Any], aws_resources: Dict[str, Any]) -> bool:
    """Store exception record to Cosmos DB."""
    if not AWS_ENABLED:
        return True

    try:
        cosmos_containers = aws_resources.get('cosmos_containers', {})
        exceptions_table = cosmos_containers.get('exceptions') or cosmos_containers.get('errors')

        if exceptions_table:
            tables = get_table_resource()
            table = cosmos_db.Table(exceptions_table)
            table.put_item(Item={
