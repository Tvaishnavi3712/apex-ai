"""
Azure ML runtime client for ApexSignal.
...
Cosmos DB data if the endpoint is unavailable (e.g. torn down to save cost) —
this keeps the demo alive when no Azure ML spend is tolerated.

Endpoint names come from `core.config.settings`. Deployed by
`backend/scripts/deploy_azure_ml_endpoints.py`.
"""
...
class PredictionBase:
    """Prediction logic shared by all clouds; transport is injected by a subclass.
    ...
    def __init__(self, region: str = "us-east-1") -> None:
        self.region = region
        self._client = None  # transport supplied by the concrete predictor
    ...
    def predict_lead_time(self, features: Dict[str, float]) -> Dict[str, Any]:
        ...
        prob = self._invoke_csv(
            endpoint=settings.AZUREML_ENDPOINT_LEAD_TIME,
            payload=csv_payload,
            default=0.15,
        )
    ...
    def predict_supplier_risk(self, moisture_pct: float, purity_pct: float, viscosity: float) -> Dict[str, Any]:
        ...
        out = self._invoke_csv(
            endpoint=settings.AZUREML_ENDPOINT_SUPPLIER,
            payload=csv_payload,
    ...
    def predict_stockout(self, features: Dict[str, float]) -> Dict[str, Any]:
        ...
        raw = self._invoke_csv(
            endpoint=settings.AZUREML_ENDPOINT_STOCKOUT,
            payload=csv_payload,
    ...
    def forecast_demand(self, series: List[float], freq: str = "W", prediction_length: int = 12) -> Dict[str, Any]:
        ...
        out = self._invoke_json(
            endpoint=settings.AZUREML_ENDPOINT_DEMAND,
            payload=payload,
            default={
                "predictions": [{
    ...
    def predict_vz_wave_risk(self, features: Dict[str, float]) -> Dict[str, Any]:
        ...
        csv_payload = ",".join(str(v) for v in ordered)
        prob = self._invoke_csv(
            endpoint=settings.AZUREML_ENDPOINT_VZ_WAVE_RISK,
            payload=csv_payload,
            default=0.18,
        )
    ...
    def predict_vz_site_cert(self, features: Dict[str, float]) -> Dict[str, Any]:
        ...
        csv_payload = ",".join(str(v) for v in ordered)
        prob = self._invoke_csv(
            endpoint=settings.AZUREML_ENDPOINT_VZ_SITE_CERT,
            payload=csv_payload,
            default=0.08,
        )
        prob_f = float(prob) if isinstance(prob, (int, float)) \
    ...
    def detect_vz_thermal_anomaly(self, series: List[float],
        ...
        if not series:
            return {
                ...
                "severity":       "none",
                "endpoint":       settings.AZUREML_ENDPOINT_VZ_THERMAL_ANOMALY,
            }
    ...
    def predict_eprod_vendor_drift(self, features: Dict[str, float]) -> Dict[str, Any]:
        ...
        csv_payload = ",".join(str(v) for v in ordered)
        raw = self._invoke_csv(
            endpoint=settings.AZUREML_ENDPOINT_EPROD_VENDOR_DRIFT,
            payload=csv_payload,
    ...
    def predict_eprod_tariff_forecast(self, features: Dict[str, float]) -> Dict[str, Any]:
        ...
        csv_payload = ",".join(str(v) for v in ordered)
        raw = self._invoke_csv(
            endpoint=settings.AZUREML_ENDPOINT_EPROD_TARIFF_FORECAST,
            payload=csv_payload,
    ...
    def predict_eprod_contract_expiry(self, features: Dict[str, float]) -> Dict[str, Any]:
        ...
        raw = self._invoke_csv(
            endpoint=settings.AZUREML_ENDPOINT_EPROD_CONTRACT_EXPIRY,
            payload=csv_payload,
    ...
    def _invoke_csv(self, endpoint, payload, default, content_type="text/csv", accept="text/csv"):
        """Transport is provided by the concrete predictor (services/azure_ml.py)."""
        return default

    def _invoke_json(self, endpoint, payload, default):
        """Transport is provided by the concrete predictor (services/azure_ml.py)."""
        return default
    ...
    def endpoint_status(self):
        """Status is provided by the concrete predictor (services/azure_ml.py)."""
        return {}

    def eprod_endpoint_status(self) -> Dict[str, str]:
        """Return {endpoint_name: status} for the 3 EPROD endpoints."""
        sm = boto3.client("azure_ml", region_name=self.region)
        names = [
            settings.AZUREML_ENDPOINT_EPROD_VENDOR_DRIFT,
            settings.AZUREML_ENDPOINT_EPROD_TARIFF_FORECAST,
            settings.AZUREML_ENDPOINT_EPROD_CONTRACT_EXPIRY,
        ]
        result = {}
        for n in names:
            try:
