...
existing Cosmos DB tables; no separate metrics store.
"""
...
router = APIRouter()
work_items_db = CosmosService(settings.TABLE_WORK_ITEMS)
agents_db = CosmosService(settings.TABLE_AGENTS)
review_decisions_db = CosmosService(settings.TABLE_REVIEW_DECISIONS)
sessions_db = CosmosService(settings.TABLE_SESSIONS)
