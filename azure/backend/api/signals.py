...
Endpoints:
  ...
  POST /api/v1/signals/risks/{event_id}/playbook   — invoke Logistics Foundry Agent Service → mitigation plan
  GET  /api/v1/signals/endpoints/status            — Azure ML endpoint statuses (for UI status badge)

Data flow:
  • /risks         reads Cosmos DB `predictive-events`, scored against Azure ML
                   lead-time endpoint for probability refresh.
  • /demand        reads Cosmos DB `purchase-orders` + historical weeks derived
  ...
  • /playbook      invokes FoundryAgentService.invoke_agent('logisticsbot', …)
...
from services.azure_ml import AzureMLPredictor
from services.foundry_agent import FoundryAgentService
from core.config import settings

router = APIRouter()

_events_db     = CosmosService(settings.TABLE_PREDICTIVE_EVENTS)
_suppliers_db  = CosmosService(settings.TABLE_SUPPLIERS)
_inv_bom_db    = CosmosService(settings.TABLE_INVENTORY_BOM)
_po_db         = CosmosService(settings.TABLE_PURCHASE_ORDERS)
_audit_db      = CosmosService(settings.TABLE_AGENT_AUDIT_LOG)

_azure_ml = AzureMLPredictor()
_foundry_agent = FoundryAgentService()
...
class PlaybookResponse(BaseModel):
    event_id: str
    status: str               # "complete" | "foundry_agent_offline"
    agent_id: str
    final_answer: str
    reasoning_steps: int
    latency_ms: int
...
async def list_risks():
    """Risk Radar tab — predictive_events with Azure ML probability refresh."""
    ...
    # Refresh probability_pct for delay-type risks by hitting Azure ML lead-time
    ...
    for r in rows:
        ...
        if r.get("type") == "delay":
            ...
            pred = _azure_ml.predict_lead_time({
...
async def demand_sensing(product_id: str = "PRD-VSD-2201"):
    ...
    pred = _azure_ml.forecast_demand(series=historical, prediction_length=12)
...
async def stockout_forecast():
    ...
    for row in inv_rows:
        ...
        pred = _azure_ml.predict_stockout({
...
async def supplier_health():
    ...
    for s in rows:
        ...
        pred = _azure_ml.predict_supplier_risk(moisture, purity, viscos)
...
async def generate_playbook(event_id: str):
    """Invoke the real Logistics Foundry Agent Service runtime with the risk context to
    ...
    result  = await _foundry_agent.invoke_agent("logisticsbot", prompt)
    latency = int((datetime.now(timezone.utc) - started).total_seconds() * 1000)

    if not result.get("success"):
        return PlaybookResponse(
            event_id=event_id,
            status="foundry_agent_offline",
            agent_id="logisticsbot",
            final_answer=result.get("response") or "Foundry Agent Service runtime unreachable.",
            reasoning_steps=0,
            latency_ms=latency,
        )
...
async def endpoints_status() -> Dict[str, str]:
    """Azure ML endpoint statuses — used by the UI to show online/offline pills."""
    return _azure_ml.endpoint_status()
...
# handlers and Foundry Agent Service agents use). Once the live Azure ML endpoints
...
async def stp_plant_reliability() -> STPPlantReliability:
    ...
    Azure ML scoring once the 3 STP endpoints are deployed.
    """
...
#   • doesn't require a real Event Grid cron (overkill for the demo)
...
# ═══════════════════════════════════════════════════════════════════════
#
# Three Azure ML endpoints (deployed by
# backend/scripts/deploy_azure_ml_vz_endpoints.py):
#
...
async def vz_wave_risk(req: VZWaveRiskRequest) -> VZWaveRiskResponse:
    """Score a wave's outage risk via Azure ML `apex-signal-vz-wave-risk`.
    ...
    pred = _azure_ml.predict_vz_wave_risk(features)
...
async def vz_site_cert(req: VZSiteCertRequest) -> VZSiteCertResponse:
    ...
    pred = _azure_ml.predict_vz_site_cert(features)
...
async def vz_site_cert_batch(req: VZSiteCertBatchRequest) -> Dict[str, Any]:
    ...
    return {"count": len(out), "scored": out, "endpoint": _azure_ml.predict_vz_site_cert.__doc__.split("·")[0].strip() if hasattr(_azure_ml.predict_vz_site_cert, "__doc__") else "apex-signal-vz-site-cert"}
...
async def vz_thermal_anomaly(req: VZThermalAnomalyRequest) -> VZThermalAnomalyResponse:
    ...
    becomes noisier).
    """
    pred = _azure_ml.detect_vz_thermal_anomaly(req.readings, ambient_temp=req.ambient_temp)
...
async def vz_endpoints_status() -> Dict[str, Any]:
    ...
    Verizon endpoints' Azure ML `EndpointStatus` so the UI can show a
    green/red dot.
    """
    import boto3
    sm_client = None  # Azure ML status is reported via AzureMLPredictor.endpoint_status()
    names = [
        settings.AZUREML_ENDPOINT_VZ_WAVE_RISK,
        settings.AZUREML_ENDPOINT_VZ_SITE_CERT,
        settings.AZUREML_ENDPOINT_VZ_THERMAL_ANOMALY,
    ]
...
# backing (Azure ML XGBoost / DeepAR / RCF) is a follow-up — swap behind
...
async def eprod_vendor_drift() -> VendorDriftResponse:
    ...
    for vid, name, msa, spend, seed_trend in base_vendors:
        ...
        # `seed_trend` for stable demo chart shape; the Azure ML call below
        ...
        # Azure ML prediction — apex-signal-eprod-vendor-drift. If the
        # endpoint is offline we fall back to a heuristic based on trailing
        # drift × 1.4 (see service `default=`).
        pred = _azure_ml.predict_eprod_vendor_drift({
...
async def eprod_tariff_forecast() -> TariffForecastResponse:
    ...
    for tid, name, docket, commodity, current, seed_change, revenue in pipelines_data:
        ...
        # Azure ML invocation — apex-signal-eprod-tariff-forecast
        pred = _azure_ml.predict_eprod_tariff_forecast({
...
async def eprod_contract_expiry() -> ContractExpiryResponse:
    ...
    for cid, vendor, ctype, days, spend, renewal, complexity, risk, scope in contracts_data:
        # Azure ML invocation — apex-signal-eprod-contract-expiry
        pred = _azure_ml.predict_eprod_contract_expiry({
