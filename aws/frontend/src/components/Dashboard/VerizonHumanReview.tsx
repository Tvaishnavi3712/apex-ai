  ...
  const decide = (kind: 'approve' | 'reject', gate: ReviewGate) => {
    ...
    if (kind === 'approve') {
      push({
        ...
        detail: 'Decision hash-chained to DynamoDB audit log · JIRA evidence attached · SES notification sent',
      });
    } else {
      push({
        tone: 'warn',
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 12, fontSize: 10.5, color: '#9ca3af', fontStyle: 'italic' }}>
        Decision hash-chained to DynamoDB audit log · IAM role: FirmwareCertApprover · JIRA evidence auto-attached
      </div>
    </div>
  );
}
