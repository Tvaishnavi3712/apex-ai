...
Mirrors the EPROD + CWFCU pattern. Azure ML / Foundry Agent Service are intentionally
...
AWS architecture story baked in: S3 + Lambda + Step Functions + Azure OpenAI +
Cosmos DB + SES + SNS + QuickSight + IAM + KMS.
"""
...
async def boler_dashboard_state() -> Dict[str, Any]:
    ...
    agents = [
        {"id": "boler-benefits-agent",  "code": "BEN", "name": "Benefits Allocation Agent",
         "description": "Benefits allocation, reclassification + exception detection · Lambda + Azure OpenAI",
         "color": "#6c47ff", "status": "Active", "status_detail": "Foundry Agent Service Connected",
        ...
        {"id": "boler-audit-agent",      "code": "AUD", "name": "Audit Lens Agent",
         "description": "Governance, lineage + Cosmos DB audit trail",
         "color": "#2563eb", "status": "Active", "status_detail": "Cosmos DB — 100% coverage",
    ...
    signal_feed = [
        ...
        {"id": "SIG-006", "tone": "approved", "tag": "COMPLETE",
         ...
         "detail": "All 847 employee records, 6 carrier files, 3 approval stages, and 7 exceptions fully logged in Cosmos DB. Audit-ready for any review."},
    ]
    ...
    aws_architecture = [
        {"layer": "File Intake",    "service": "Azure Blob Storage (restricted bucket)",
         "role":  "Carrier files dropped by HR/Benefits team"},
        {"layer": "Extraction",     "service": "Azure Functions + Azure OpenAI",
        ...
        {"layer": "Storage",        "service": "Azure Cosmos DB",
         "role":  "Audit ledger, exception records, allocation history"},
        {"layer": "Notifications",  "service": "Azure Communication Services + SNS",
        ...
        {"layer": "AI/LLM",          "service": "Azure OpenAI (Claude)",
         "role":  "Agent intelligence, exception reasoning, narrative generation"},
        {"layer": "Security",       "service": "Microsoft Entra ID + KMS",
...
async def boler_apply_reclass(exc_id: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "toast": {
            ...
            "detail": "Cosmos DB audit logged · downstream JEs regenerated · SES notification sent",
        },
        "audit_entry": entry,
    }
...
async def boler_remove_allocation(exc_id: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "toast": {
            ...
            "detail": "Catch-up credit applied to division JE · Cosmos DB audit logged",
        },
        "audit_entry": entry,
    }
...
async def boler_dismiss(exc_id: str, reason: str = "no_action_needed") -> Dict[str, Any]:
    ...
    return {
        ...
        "toast": {
            ...
            "detail": f"Reason: {reason} · entry persists in Cosmos DB audit log",
        },
        "audit_entry": entry,
    }
...
# ─────────────────────────────────────────────────────────────────────
# Signal individual endpoints (deterministic — no Azure ML)
# ─────────────────────────────────────────────────────────────────────
