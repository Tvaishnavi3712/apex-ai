"""
Multi-LLM Model Registry — single source of truth for which Azure OpenAI models
...
Surfaces in three places:
  ...
  2. Foundry Agent Service agent runtimes — read PRESETS or per-agent overrides from the
     invocation payload to switch AzureOpenAIModel at tool-call vs synthesis time.
  3. Audit Lens — looks up display_name + per-1M token cost to render
     the per-step model chip and the per-query cost estimate.

USD pricing as of 2026-04 from us-east-1 Azure OpenAI public pricing.
...
# ---------------------------------------------------------------------------
# Available models. Azure OpenAI IDs use the inference-profile prefix (us.) where
# required — same convention as the existing Foundry Agent Service deployments.
# ---------------------------------------------------------------------------
@dataclass(frozen=True)
class ModelSpec:
    """A single LLM choice exposed in the Settings dropdowns."""
    id: str                          # Azure OpenAI model id (used by AzureOpenAIModel())
    ...
    requires_inference_profile: bool = True  # most Azure OpenAI models need 'us.' prefix
...
# The platform supports more (any Azure OpenAI model id works), but the dropdown
...
MODELS: List[ModelSpec] = [
    ...
    # GPT-class models alongside Azure OpenAI. Routing layer needs an API-key
...
def get_model(model_id: str) -> Optional[ModelSpec]:
    """Look up a ModelSpec by Azure OpenAI id. Returns None if not in registry."""
