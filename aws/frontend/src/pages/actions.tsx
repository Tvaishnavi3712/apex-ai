...
export default function ActionsPage() {
  ...
  //   • DynamoDB  (/actions/)                              — source of truth for
  ...
  // but inherits runs count from DynamoDB. Actions present only in DynamoDB
  ...
  const q = useQuery<{ items: ActionItem[]; source: DataSource }>({
    ...
    queryFn: async () => {
      ...
      // If DynamoDB was the sole contributor, label it 'api'; otherwise the
  ...
  //     (cbb-*) which are AgentCore Strands tools, not Lambda handlers,
  ...
  // are now seeded into DynamoDB via /api/v1/actions/seed-agentic-enterprise
  ...
  const all: ActionItem[] = useMemo(() => {
    ...
    // Prefixes:
    //   • cbb-*  (CBB AgentCore Strands tools — not Lambda handlers)
    ...
    <>
      ...
      <DataSourceBanner
        ...
        hint={source === 'mock'
          ...
          : 'Schemas + descriptions come from the handler files. Run curl -X POST http://localhost:8000/api/v1/actions/registry/register-all to also persist invocation counts to DynamoDB.'}
      />
...
const HINT_FOR_ERROR: Record<NonNullable<InvokeEnvelope['error_type']>, string> = {
  ...
  runtime:
    "The handler ran but raised an exception. Inspect the traceback — common culprits are missing AWS credentials or missing DynamoDB tables in local dev.",
};
 ...
 * `apex-<industry>-<action_name>`; store actions also get a DynamoDB link.
 *
...
function awsConsoleUrl(action: ActionItem): string {
  ...
  if (action.type === 'store') {
    return `https://${region}.console.aws.amazon.com/dynamodbv2/home?region=${region}#table?name=ApexResults`;
  }
    ...
    <div style={{ padding: 16 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
        <MiniStat
          ...
          label="Total Runs"
          hint="From DynamoDB invocation_count"
        />
        <MiniStat
...
/** Hit /actions/ (DynamoDB) and return a flat list. */
 ...
 * or DynamoDB) matches one of these names, the UI will tag it with the
...
const SAMPLE_ACTIONS: ActionItem[] = [
  ...
  { id: 'cbb-2',  name: 'athena_federated_query',      description: 'Run a federated SQL query against CBB\'s S3 data lake via Amazon Athena',                                     type: 'execute',  industry: 'supply_manufacturing', runs: 7840,  active: true, section: 'CBB Actions', sectionKey: 'supply_manufacturing', usedInDemos: [1,3] },
  ...
  /* ═══ Generic actions ═══ */
  // Core
  { id: 'core-1', name: 'BDA Document Extraction',   description: 'Extract structured fields from documents using Amazon Bedrock Data Automation', type: 'extract',  industry: 'all', runs: 24891, active: true, section: 'Core Actions', sectionKey: 'all' },
  ...
  { id: 'core-5', name: 'DynamoDB Result Writer',     description: 'Persist extraction results and audit trail to DynamoDB with TTL support', type: 'store', industry: 'all', runs: 15220, active: true, section: 'Core Actions', sectionKey: 'all' },
  { id: 'core-6', name: 'Lambda Function Invoker',    description: 'Invoke custom AWS Lambda functions with typed input/output schema mapping', type: 'execute', industry: 'all', runs: 5890, active: true, section: 'Core Actions', sectionKey: 'all' },
