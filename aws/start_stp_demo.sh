...
#   1. Verifies AWS creds + 5 STP AgentCore runtimes are READY
...
echo -e "${BLUE}[2/5]${NC} Verifying 5 AgentCore runtimes…"

# Quick, single-call list — much faster than 5 individual get-agent-runtime calls
runtimes_json=$(aws bedrock-agentcore-control list-agent-runtimes --region us-east-1 2>/dev/null)
...
if [ "$ready_count" != "5" ]; then
  ...
  echo -e "  ${DIM}Re-deploy: cd agentcore-agents && python3 deploy_stp_agents.py --apply${NC}"
  exit 1
fi
...
if [ "$QUICK" = 0 ]; then
  ...
  (cd agentcore-agents && python3 prewarm_stp_runtimes.py --once 2>&1) | sed 's/^/    /'

  # Then keep it running every 4 min
  (cd agentcore-agents && nohup python3 prewarm_stp_runtimes.py --watch \
