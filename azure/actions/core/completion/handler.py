...
async def completion(
    ...
    Context keys used:
        - aws_resources: Storage configuration (Cosmos DB tables, blob containers)
...
async def _store_results(
    ...
    """Store workflow results to Cosmos DB and/or S3."""
    if not AWS_ENABLED:
        return True, "mock://results"

    try:
        # Store to Cosmos DB if table configured
        cosmos_containers = aws_resources.get('cosmos_containers', {})
        results_table = cosmos_containers.get('workflow_results') or cosmos_containers.get('results')

        if results_table:
            tables = get_table_resource()
            table = cosmos_db.Table(results_table)
            table.put_item(Item={
                'workflow_id': workflow_id,
                'completed_at': datetime.utcnow().isoformat(),
            ...
            return True, f"cosmos_db://{results_table}/{workflow_id}"
