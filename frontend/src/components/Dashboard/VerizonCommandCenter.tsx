...
export function VerizonCommandCenter() {
  ...
  // ──────────────────── Predictive Intelligence — Azure ML wiring ──────────────
  ...
  return (
    <div style={{ background: '#f0f4f8', minHeight: '100vh', color: '#1e293b', padding: '28px 32px', maxWidth: 1600, margin: '0 auto' }}>
      ...
      {/* Bottom row: Predictive Intelligence panel — live Azure ML output */}
      <div style={{ ...panel, padding: '18px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#0f172a' }}>Predictive Intelligence</div>
            <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 1 }}>
              Real-time wave risk scoring · 3 Azure ML endpoints · {vzStatusQuery.data?.endpoints?.filter(e => e.status === 'InService').length ?? 0}/3 InService
            </div>
          </div>
          <span style={chip(
