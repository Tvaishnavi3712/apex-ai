...
Provides type-safe schema building for AgentCore Gateway
"""
...
class ActionInputSchema:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to JSON Schema format for AgentCore Gateway"""
        required_fields = [
...
def document_extraction_input() -> ActionInputSchema:
    ...
    return ActionInputSchema(
        ...
        "document_s3_uri", "S3 URI of the document to process", required=True
    ).add_string(
        "blueprint_arn", "ARN of the BDA blueprint to use", required=False
    ).add_string(
        "output_s3_uri", "S3 URI for output (optional)", required=False
    )
