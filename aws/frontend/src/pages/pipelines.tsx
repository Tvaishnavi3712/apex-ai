...
export default function PipelinesPage() {
  ...
  // sees them even when only some pipelines are seeded in DynamoDB.
  ...
  const pipelinesWithLiveHistory: Pipeline[] = useMemo(() => {
    ...
    const triggerSuffix = latest.trigger === 's3_event' ? ' (S3 trigger)' : '';
...
function schemaSamplesFor(kind: StageKind): { input: SchemaRow[]; output: SchemaRow[] } {
  const base: Record<string, { input: SchemaRow[]; output: SchemaRow[] }> = {
    extract: {
      input:  [
        { name: 'document_uri',  type: 'string', required: true,  description: 'S3 URI of the source document' },
 ...
 * `value_map` blob from DynamoDB (patched onto the 3 CBB playbooks by the
