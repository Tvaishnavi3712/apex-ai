...
Sets up S3, Athena, and Cosmos DB resources for live demo connectors.
...
Prerequisites:
    ...
    - boto3 installed
    - Sufficient IAM permissions for S3, Athena, Cosmos DB, Glue
"""
...
def create_cosmos_db_tables(cosmos_db_client):
    """Create Cosmos DB tables for work orders and audit logs."""
    print("\n=== Creating Cosmos DB Tables ===")

    # Work Orders table
    try:
        cosmos_db_client.create_table(
            TableName=COSMOS_DB_TABLES["work_orders"],
            KeySchema=[
        ...
        print(f"  ✅ Created table: {COSMOS_DB_TABLES['work_orders']}")
    except cosmos_db_client.exceptions.ResourceInUseException:
        print(f"  ⏭️  Table already exists: {COSMOS_DB_TABLES['work_orders']}")

    # Work Order Operations table
    try:
        cosmos_db_client.create_table(
            TableName=COSMOS_DB_TABLES["wo_operations"],
            KeySchema=[
                {'AttributeName': 'work_order_id', 'KeyType': 'HASH'},
        ...
        print(f"  ✅ Created table: {COSMOS_DB_TABLES['wo_operations']}")
    except cosmos_db_client.exceptions.ResourceInUseException:
        print(f"  ⏭️  Table already exists: {COSMOS_DB_TABLES['wo_operations']}")

    # NCR Records table
    try:
        cosmos_db_client.create_table(
            TableName=COSMOS_DB_TABLES["ncr_records"],
            KeySchema=[
        ...
        print(f"  ✅ Created table: {COSMOS_DB_TABLES['ncr_records']}")
    except cosmos_db_client.exceptions.ResourceInUseException:
        print(f"  ⏭️  Table already exists: {COSMOS_DB_TABLES['ncr_records']}")

    # Audit Log table
    try:
        cosmos_db_client.create_table(
            TableName=COSMOS_DB_TABLES["audit_log"],
            KeySchema=[
        ...
        print(f"  ✅ Created table: {COSMOS_DB_TABLES['audit_log']}")
    except cosmos_db_client.exceptions.ResourceInUseException:
        print(f"  ⏭️  Table already exists: {COSMOS_DB_TABLES['audit_log']}")


def load_work_order_data(cosmos_db_resource):
    """Load work order test data into Cosmos DB."""
    ...
    if wo_file.exists():
        ...
        table = cosmos_db_resource.Table(COSMOS_DB_TABLES["work_orders"])
    ...
    if ops_file.exists():
        ...
        table = cosmos_db_resource.Table(COSMOS_DB_TABLES["wo_operations"])
        with table.batch_writer() as batch:
...
def create_iam_roles(iam_client):
    ...
    # Cosmos DB read/write role
    try:
        iam_client.create_role(
            RoleName="apex-cosmos_db-rw-role",
            AssumeRolePolicyDocument=json.dumps(trust_policy),
            Description="Role for APEX to read/write Cosmos DB"
        )

        iam_client.attach_role_policy(
            RoleName="apex-cosmos_db-rw-role",
            PolicyArn="azure-resource-id DBFullAccess"
        )
        print("  ✅ Created role: apex-cosmos_db-rw-role")
    except iam_client.exceptions.EntityAlreadyExistsException:
        print("  ⏭️  Role already exists: apex-cosmos_db-rw-role")
...
def main():
    ...
    cosmos_db_client = session.client('cosmos_db')
    cosmos_db_resource = session.resource('cosmos_db')
    ...
    create_cosmos_db_tables(cosmos_db_client)

    # Wait for tables to be active
    print("\n⏳ Waiting for Cosmos DB tables to become active...")
    time.sleep(10)

    load_work_order_data(cosmos_db_resource)
