...
def convert_for_cosmos_db(obj: Any) -> Any:
    """Convert floats to Decimal and dates to strings for Cosmos DB compatibility"""
    ...
    if isinstance(obj, float):
        ...
        return {k: convert_for_cosmos_db(v) for k, v in obj.items()}
    elif isinstance(obj, list):
        return [convert_for_cosmos_db(i) for i in obj]
    return obj
...
router = APIRouter()
db = CosmosService(settings.TABLE_PLAYBOOKS)
...
async def list_playbooks(
    ...
    # A cold scan of this container is slow; cache it briefly so browsing
    # the gallery does not re-scan on every page load.
    items = await _list_cache.get_or_set(cache_key, _load)

    # Re-seeding an industry can create a second row for a playbook that
    # already exists. Collapse duplicates — newest wins.
    before = len(items)
    ...
    if skipped:
        ...
        logging.getLogger(__name__).warning(
            "list_playbooks: skipped %d malformed legacy items", skipped,
        )
    return out
...
async def deploy_playbook(playbook_id: str, environment: str = "development"):
    """Deploy a playbook to Azure AI Foundry Agent Service"""
    playbook = await get_playbook(playbook_id)

    # TODO: Implement Foundry Agent Service deployment
    # 1. Parse playbook to Foundry Agent Service configuration
    # 2. Create/update Azure OpenAI Agent
...
async def seed_playbooks(
    ...
    for yaml_file in yaml_files:
        try:
            ...
            # Convert floats to Decimal and dates to strings for Cosmos DB
            playbook_data = convert_for_cosmos_db(playbook_data)
...
async def warm_list_cache() -> None:
    """Prime the default gallery query at startup so the first load is instant."""
