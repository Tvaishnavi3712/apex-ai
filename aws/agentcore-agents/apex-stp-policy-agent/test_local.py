...
Exercises the tools directly (no Bedrock call) and asserts the demo question's
required citation appears in the search results. Also runs the entrypoint
end-to-end if AWS credentials are configured for Bedrock.
...
def test_entrypoint() -> None:
    """Optional: full Bedrock roundtrip. Requires AWS creds + Bedrock access."""
    if os.environ.get("APEX_SKIP_BEDROCK") == "1":
        print("[bedrock] skipped (APEX_SKIP_BEDROCK=1)")
        return
    print("[bedrock] invoking entrypoint with demo prompts")
    for p in DEMO_PROMPTS:
        try:
            ...
            print(f"  [skip] bedrock call failed: {e}")
            return
