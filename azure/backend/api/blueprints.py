...
Manage Azure AI Document Intelligence blueprints
"""
...
router = APIRouter()
db = CosmosService(settings.TABLE_BLUEPRINTS)
s3 = BlobService(settings.CONTAINER_BLUEPRINTS)
...
async def list_blueprints(
    ...
    # Tolerate legacy Cosmos DB rows that pre-date the current Blueprint
...
RESPOND WITH JSON ONLY. NO MARKDOWN CODE BLOCKS."""
        ...
        # Call Claude
        model_id = os.environ.get('AZURE_OPENAI_DEPLOYMENT_DEFAULT', 'us.anthropic.claude-opus-4-6-v1')
...
async def seed_blueprints(
    ...
    for json_file in json_files:
        try:
            ...
            #   (1) Document schema wrapper: { documentSchema: { description, fields|properties, ... } }
            ...
            if 'documentSchema' in blueprint_json or 'bdaSchema' in blueprint_json:
                # (1) BDA wrapper
                bda_schema = blueprint_json.get('documentSchema') or blueprint_json['bdaSchema']
