...
Inspired by Sema4.ai's action decorators but designed for Foundry Agent Service
"""
...
class ApexActionSchema:
    ...
    def to_gateway_schema(self) -> Dict[str, Any]:
        """Convert to Foundry Agent Service tool schema format"""
        schema = {
            "name": self.name,
...
def apex_action(schema: ApexActionSchema):
    ...
    - Registration with Foundry Agent Service
    - Schema validation
...
def create_lambda_handler(action_func: Callable) -> Callable:
    ...
    - Foundry Agent Service context extraction
    - Error handling
    - Response formatting
    ...
    def handler(event, context):
        try:
            # Extract tool context from Foundry Agent Service
            tool_context = {}
