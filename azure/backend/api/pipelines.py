...
Cosmos DB table — pipelines are computed on the fly from existing playbooks.
"""
...
from services.cosmos import CosmosService as _DDB_DETAILS
from core.config import settings as _settings_DETAILS

_pipelines_detail_db_v2 = _DDB_DETAILS(_settings_DETAILS.TABLE_PIPELINES)
...
# match the Cosmos DB playbook keys (`pb-*`). The seeder patches the value_map
...
async def get_pipeline_value_map(
    ...
    Lookup order:
      1. Direct Cosmos DB playbook row keyed by `pipeline_id`.
    ...
    try:
        ...
        # Infra issue (Cosmos DB down, network, credential) — don't dead-end
        # the UI; fall through to synthesis. Logging is handled upstream by
        # the CosmosService wrapper which logs on each failure.
        pb = None
