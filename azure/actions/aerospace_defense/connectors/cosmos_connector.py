"""
Cosmos DB Connector for GovCloud ERP Replacement
...
class CosmosConnector:
    """Connector for work order management via Cosmos DB."""

    def __init__(self, region: str = REGION):
        self._resource = get_table_resource()
        self.tables = {name: self._resource.Table(table_name)
...
@apex_action(
    ...
    description="Query work order details from ERP (Cosmos DB)",
...
def query_work_order(work_order_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            **work_order,
            "source": "GovCloud ERP (Cosmos DB)",
            "connector_mode": "READ"
        }
    except Exception as e:
        return {"error": str(e), "source": "GovCloud ERP (Cosmos DB)"}
