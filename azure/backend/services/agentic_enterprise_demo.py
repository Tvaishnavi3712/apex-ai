...
Design rules followed (per the user's reminder):
  ...
  2. The agents are deterministic Python (not real LLM calls) so the
     ...
     If/when real Foundry Agent Service runtimes get deployed for these agents,
     swap the routing in services/foundry_agent.py and this module becomes
     the offline fallback.

  3. Every agent returns the standard runtime shape:
...
class OrchestratorAgent:
    ...
    def respond(cls, prompt: str) -> Dict[str, Any]:
        """Run the full plan and return an runtime-shaped response."""
...
class LeaseAgent:
    ...
    def _tool_lease_extract(text: Optional[str] = None) -> Dict[str, Any]:
        ...
        Textract / Azure OpenAI; this in-process version is deterministic and
        fast for the demo.
        """
        if text is None:
