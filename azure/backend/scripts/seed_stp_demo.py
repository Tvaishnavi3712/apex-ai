#!/usr/bin/env python3
"""
Seed STP Phase 2 demo content into Cosmos DB.
...
from services.cosmos import CosmosService  # noqa: E402
...
async def resolve_playbook_uuids() -> Dict[str, str]:
    ...
    """
    db = CosmosService(settings.COSMOS_DB_PLAYBOOKS)
...
async def main(force: bool) -> int:
    print(f"╔═══════════════════════════════════════════╗")
    print(f"║  STP Phase 2 — Cosmos DB seeder            ║")
    print(f"║  force={force}                              ║")
    print(f"╚═══════════════════════════════════════════╝\n")
    ...
    pipelines_table = getattr(settings, "COSMOS_DB_PIPELINES", "apex-ai-platform-pipelines")
    await upsert_table(
        pipelines_table,
        "pipeline_id",
        STP_PIPELINES,
    ...
    review_table = getattr(settings, "COSMOS_DB_REVIEW_QUEUE", "apex-ai-platform-review-queue")
    await upsert_table(
        review_table,
        "review_id",
        STP_REVIEW_ITEMS,
