...
Delegation strategy:
  1. PRIMARY — invoke the specialist's Foundry Agent Service runtime via boto3
     (`azure_openai-foundry_agent` invoke_agent_runtime). Set the runtime ARN per
...
_ROOT = Path(__file__).resolve().parent.parent  # foundry_agent-agents/
...
def _delegate(name: str, query: str) -> Dict[str, Any]:
    """Invoke a specialist by name. Foundry Agent Service ARN first, in-process fallback.

    Forwards `_CURRENT_MODEL_OVERRIDES` so each specialist's AzureOpenAIModel
    ...
    if arn:
        try:
            import boto3
            client = boto3.client("azure_openai-foundry_agent", region_name=REGION)
            ...
            return _ok({"specialist": name, "via": "foundry_agent", "arn": arn, "response": parsed})
        except Exception as e:  # noqa: BLE001
            return _err(f"Foundry Agent Service invoke for '{name}' failed: {e}")
    # Fallback: in-process
    try:
...
def _resolve_router_model(model_overrides: Dict[str, Dict[str, str]] | None) -> str:
    """Pick which AzureOpenAIModel id powers the router's classification + delegation."""
...
def _get_agent(routing_model_id: str) -> Agent:
    ...
    model = AzureOpenAIModel(model_id=routing_model_id, region_name=REGION)
    agent = Agent(
        model=model,
