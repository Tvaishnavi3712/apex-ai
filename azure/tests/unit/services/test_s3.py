...
class TestBlob StorageService:
    """Tests for Blob StorageService class."""

    @pytest.fixture
    def mock_bucket(self):
        """Create a mock Blob Storage bucket."""
        with mock_s3():
    ...
    def service(self, mock_bucket):
        """Create Blob StorageService instance with mocked bucket."""
        from backend.services.s3 import Blob StorageService

        with patch.object(Blob StorageService, "__init__", lambda self, bucket_name: None):
            svc = Blob StorageService.__new__(Blob StorageService)
...
class TestBlob StorageServiceErrors:
    """Test error handling in Blob StorageService."""
