...
def refund_process(
    ...
    try:
        ...
        refunds_table = cosmos_db.Table(os.environ.get('REFUNDS_TABLE', 'apex-retail-refunds'))
        transactions_table = cosmos_db.Table(os.environ.get('TRANSACTIONS_TABLE', 'apex-retail-transactions'))
...
def _issue_store_credit(customer_id: str, amount: float) -> dict:
    ...
    try:
        tables = get_table_resource()
        table = cosmos_db.Table(os.environ.get('STORE_CREDITS_TABLE', 'apex-store-credits'))
        table.put_item(Item={
