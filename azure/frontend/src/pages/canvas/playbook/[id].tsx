 ...
 *   1. /api/v1/playbooks/{id}  — real Cosmos DB row (always tried first)
 ...
 * (e.g. an Blob container path with a recursive glob for AWS S3). All categories are
...
function TriggerPicker({ onClose, onPick }: {
  ...
  const orderedCategories = [
    ...
    'Industrial / OT', 'Financial & Compliance', 'Azure Services',
...
function _apiToBlueprintRef(api: any, playbookActions: ActionEntry[]): BlueprintRef {
  ...
  return {
    ...
    fields,
    arn:            api.docintel_blueprint_id || api.arn || '',
  };
}
  ...
  return {
    ...
    triggers:   Array.isArray(api.triggers) ? api.triggers.map((t: any) => {
      ...
      const TYPE_LABEL: Record<string, string> = {
        ...
        eventbridge: 'Event Grid', sqs: 'SQS', schedule: 'Schedule', api: 'API',
      };
      return {
...
- Engine:  Azure Synapse (federated BOM graph)
...
- Window: lookback_days=14 (cached for tonight; live Azure ML tomorrow)
...
- Any flag → procurement_exceptions queue with reason codes`,
    ...
    triggers: [
      { label: 'SAP PO release',  detail: 'Event Grid SAP.PO.Released',          active: true  },
...
- All gates clean → auto_approve + audit pack archived`,
    ...
    triggers: [
      { label: 'ServiceNow request', detail: 'Event Grid ServiceNow.NonPORequest',  active: true  },
...
const DEFAULT_PLAYBOOK: PlaybookData = {
  ...
  industry_label: '',
  description: 'No playbook matches this id, and the platform API is unreachable. Verify the id in the URL, or seed the playbook into Cosmos DB via POST /api/v1/playbooks/seed?industry=<industry>.',
