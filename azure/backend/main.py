...
from api import playbooks, agents, blueprints, actions, documents, work_items, chat, voice, pipelines, metrics, azure_admin, simulator, signals, llm, review_queue, telecommunications_cycle, eprod_cycle, cwfcu, boler, inventory, jobs, workflow_import
...
DEFAULT_AGENTS = [
    ...
    # reads from synthetic-data/agentic_enterprise/*.json. No agent runtime
...
async def lifespan(app: FastAPI):
    ...
    # Prime the gallery list caches so the first Canvas load is instant.
...
app.include_router(azure_admin.router, prefix="/api/v1/azure", tags=["Azure Admin"])
