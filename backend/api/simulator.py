...
Scenarios live in Cosmos DB (`apex-ai-platform-simulator-scenarios`). Running a
scenario invokes the real Logistics Foundry Agent Service runtime with a structured prompt
that includes the scenario context; we return the Foundry Agent Service reasoning + final
...
router = APIRouter()
scenarios_db = CosmosService(settings.TABLE_SIMULATOR_SCENARIOS)
...
class Scenario(BaseModel):
    ...
    director:        str
    agent_id:        str  = "logisticsbot"           # routes to Logistics Foundry Agent Service runtime
...
def _to_ddb(value: Any) -> Any:
    """Walk a dict/list tree and convert floats to Decimal — Cosmos DB rejects float."""
...
async def create_scenario(payload: ScenarioInput):
    """Inject a new crisis scenario. Persists to Cosmos DB so it survives
...
async def run_scenario(scenario_id: str):
    """Inject the scenario into the Logistics Foundry Agent Service runtime and return its
    ...
    result = await foundry_agent.invoke_agent(scenario.agent_id, prompt)
