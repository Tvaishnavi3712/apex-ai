...
def ehr_update(
    ...
    try:
        ...
        patients_table = cosmos_db.Table(os.environ.get('PATIENTS_TABLE', 'apex-patients'))
        audit_table = cosmos_db.Table(os.environ.get('AUDIT_TABLE', 'apex-audit'))
...
def _build_update_expression(update_type: str, data: dict) -> tuple:
    """Build Cosmos DB update expression"""
    expressions = []
    attr_values = {}
