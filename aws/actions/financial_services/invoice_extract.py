...
Extract structured data from invoices using Bedrock Data Automation
"""
...
@apex_action(ApexActionSchema(
    ...
    input_schema=ActionInputSchema(description="Invoice extraction parameters")
        .add_string("document_s3_uri", "S3 URI of the invoice document", required=True)
        .add_string("blueprint_arn", "BDA blueprint ARN (optional, uses default)", required=False)
        .add_string("output_s3_uri", "S3 URI for extraction output", required=False)
...
def invoice_extract(
    ...
    """
    Extract structured data from an invoice using Bedrock Data Automation

    Args:
        document_s3_uri: S3 URI of the invoice (s3://bucket/key)
...
def get_extraction_result(invocation_arn: str) -> dict:
    ...
    try:
        ...
        if status == 'Success':
            ...
            # Parse output S3 URI and fetch results
