...
export default function CanvasPage() {
      ...
      try {
        ...
        if (r.ok) {
          ...
          const items: BlueprintItem[] = raw.map((b) => ({
            ...
            status:      b.status,
            arn:         b.docintel_blueprint_id || b.arn,
          }));
          if (items.length > 0) return { items, source: 'api' as DataSource };
        }
      } catch { /* fall through */ }
  ...
  // and don't live in Cosmos DB. (2) The seeder has a habit of inserting the
...
const CBB_PLAYBOOKS: PlaybookItem[] = [
  ...
  // Cosmos DB (full_certification_cycle, schema_drift_response, operator_kb_query,
...
const GENERIC_SAMPLE_PLAYBOOKS: PlaybookItem[] = [
  { id: 'pb-1',  name: 'Invoice Processing & Validation', industry: 'financial_services',    status: 'deployed', description: 'Azure AI Document Intelligence · 8 actions · Last run 2 min ago' },
  { id: 'pb-2',  name: 'Loan Application Intake',         industry: 'financial_services',    status: 'deployed', description: 'Foundry Agents · 12 actions · Last run 1 hr ago' },
  { id: 'pb-3',  name: 'KYC Document Verification',       industry: 'financial_services',    status: 'draft',    description: 'Azure AI Document Intelligence · 6 actions · Never run' },
  { id: 'pb-4',  name: 'Claims Intake & Triage',          industry: 'insurance_underwriting',status: 'deployed', description: 'Foundry Agents · 14 actions · Last run 30 min ago' },
  { id: 'pb-5',  name: 'CRE Underwriting Submission',     industry: 'insurance_underwriting',status: 'deployed', description: 'Foundry Agents · 10 actions · Last run 4 hr ago' },
  { id: 'pb-6',  name: 'RFP Response Generation',         industry: 'aerospace_defense',     status: 'deployed', description: 'Foundry Agents · 9 actions · Last run 6 hr ago' },
  { id: 'pb-7',  name: 'CNC Code Review & Optimization',  industry: 'aerospace_defense',     status: 'idle',     description: 'Foundry Agents · 7 actions · Last run 2 days ago' },
  { id: 'pb-8',  name: 'Production Line QC Monitoring',   industry: 'supply_manufacturing',         status: 'deployed', description: 'Foundry Agents · 9 actions · Last run 18 min ago' },
  { id: 'pb-9',  name: 'Supplier PO Matching',            industry: 'supply_manufacturing',         status: 'deployed', description: 'Azure AI Document Intelligence · 6 actions · Last run 42 min ago' },
  { id: 'pb-10', name: 'Work Order Routing & Scheduling', industry: 'supply_manufacturing',         status: 'draft',    description: 'Foundry Agents · 8 actions · Never run' },
];

const CBB_BLUEPRINTS: BlueprintItem[] = [
  { id: 'bp-cbb-order',    name: 'Order Modification Form v2.1', industry: 'supply_manufacturing', field_count: 12, status: 'deployed', arn: 'azure-docintel/CBB/blueprint/order-mod-v2.1' },
  { id: 'bp-cbb-qc',       name: 'QC Certificate v3.0',          industry: 'supply_manufacturing',      field_count: 18, status: 'deployed', arn: 'azure-docintel/CBB/blueprint/qc-cert-v3.0' },
  { id: 'bp-cbb-disrupt',  name: 'Supplier Delay Alert v1.0',    industry: 'supply_manufacturing',       field_count: 14, status: 'deployed', arn: 'azure-docintel/CBB/blueprint/supplier-alert-v1.0' },
  // STP Phase 2 blueprints — 6 document types covering the 4 use cases.
  { id: 'bp-stp-hr-policy',     name: 'STP HR Policy Document',   industry: 'nuclear_operations', field_count: 14, status: 'deployed', arn: 'azure-docintel/STP/blueprint/hr-policy-v1' },
  { id: 'bp-stp-procedure',     name: 'STP Plant Procedure',       industry: 'nuclear_operations', field_count: 18, status: 'deployed', arn: 'azure-docintel/STP/blueprint/procedure-v1' },
  { id: 'bp-stp-work-order',    name: 'STP Work Order Record',     industry: 'nuclear_operations', field_count: 16, status: 'deployed', arn: 'azure-docintel/STP/blueprint/work-order-v1' },
  { id: 'bp-stp-work-package',  name: 'STP Scanned Work Package',  industry: 'nuclear_operations', field_count: 22, status: 'deployed', arn: 'azure-docintel/STP/blueprint/work-package-v1' },
  { id: 'bp-stp-incident',      name: 'STP Incident / CR Report',  industry: 'nuclear_operations', field_count: 17, status: 'deployed', arn: 'azure-docintel/STP/blueprint/incident-v1' },
  { id: 'bp-stp-sensor',        name: 'STP Sensor Telemetry Schema', industry: 'nuclear_operations', field_count: 6,  status: 'deployed', arn: 'azure-docintel/STP/blueprint/sensor-v1' },
  // Telecommunications — Verizon Far Edge POC (5 blueprints mirroring the JSON on disk).
  { id: 'bp-tel-robot',     name: 'ROBOT Framework Test Output',          industry: 'telecommunications', field_count: 12, status: 'deployed', arn: 'azure-docintel/VZ/blueprint/robot-framework-v1' },
  { id: 'bp-tel-redfish',   name: 'Redfish Schema Diff Analyzer',         industry: 'telecommunications', field_count:  8, status: 'deployed', arn: 'azure-docintel/VZ/blueprint/redfish-schema-v1' },
  { id: 'bp-tel-runbook',   name: 'Upgrade Runbook Extractor',            industry: 'telecommunications', field_count: 10, status: 'deployed', arn: 'azure-docintel/VZ/blueprint/upgrade-runbook-v1' },
  { id: 'bp-tel-kb',        name: 'Known Issues KB Article',              industry: 'telecommunications', field_count: 11, status: 'deployed', arn: 'azure-docintel/VZ/blueprint/known-issues-v1' },
  { id: 'bp-tel-cert-rpt',  name: 'Certification Report Generator',       industry: 'telecommunications', field_count: 18, status: 'deployed', arn: 'azure-docintel/VZ/blueprint/certification-report-v1' },
  // Oil & Gas Midstream — EPROD POC blueprints (4 proposal + 2 wow).
  { id: 'bp-eprod-invoice',  name: 'EPROD Vendor Invoice',                industry: 'oil_gas_midstream', field_count: 14, status: 'deployed', arn: 'azure-docintel/EPROD/blueprint/invoice-v1' },
  { id: 'bp-eprod-po',       name: 'EPROD Purchase Order',                industry: 'oil_gas_midstream', field_count: 16, status: 'deployed', arn: 'azure-docintel/EPROD/blueprint/purchase-order-v1' },
  { id: 'bp-eprod-msa',      name: 'EPROD Master Service Agreement',      industry: 'oil_gas_midstream', field_count: 12, status: 'deployed', arn: 'azure-docintel/EPROD/blueprint/msa-v1' },
  { id: 'bp-eprod-quote',    name: 'EPROD Engineering Quote',             industry: 'oil_gas_midstream', field_count: 13, status: 'deployed', arn: 'azure-docintel/EPROD/blueprint/engineering-quote-v1' },
  { id: 'bp-eprod-tariff',   name: 'EPROD FERC Tariff Sheet',             industry: 'oil_gas_midstream', field_count: 15, status: 'deployed', arn: 'azure-docintel/EPROD/blueprint/ferc-tariff-v1' },
  { id: 'bp-eprod-jib',      name: 'EPROD JIB Statement',                 industry: 'oil_gas_midstream', field_count: 14, status: 'deployed', arn: 'azure-docintel/EPROD/blueprint/jib-statement-v1' },
];

const GENERIC_SAMPLE_BLUEPRINTS: BlueprintItem[] = [
  { id: 'bp-1', name: 'Invoice Blueprint',           industry: 'financial_services',    field_count: 12, status: 'deployed', arn: 'azure-resource' },
  { id: 'bp-2', name: 'Claim Form Blueprint',        industry: 'insurance_underwriting',field_count: 18, status: 'deployed', arn: 'azure-resource' },
  { id: 'bp-3', name: 'Defense Contract Blueprint',  industry: 'aerospace_defense',     field_count: 24, status: 'draft' },
  { id: 'bp-4', name: 'Bill of Materials v2',        industry: 'supply_manufacturing',         field_count: 22, status: 'deployed', arn: 'azure-resource' },
  { id: 'bp-5', name: 'Purchase Order (Supplier) v1', industry: 'supply_manufacturing',        field_count: 16, status: 'deployed', arn: 'azure-resource' },
 ...
 *      what makes the platform feel "live" — Cosmos DB rows are the truth.
 ...
 * live UUIDs from Cosmos DB. Both rendered. Now: API wins.
 */
