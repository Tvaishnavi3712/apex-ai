...
async def get_message_dvr(session_id: str, message_id: str):
    ...
    specific assistant message. The data is whatever the AgentCore runtime
...
class SendMessageBody(BaseModel):
    ...
    to the AgentCore runtime payload.
    """
...
async def send_message(
    ...
    Body fields (all optional):
      ...
      • attachments     — list of S3 URIs / file ids
    """
    ...
    # so the AgentCore payload stays comfortably under the 100KB CLI limit.
    recent_turns = [
    ...
    agent_result = await agentcore.invoke_agent(
        agent_name,
        content,
        session_id=session_id,
...
async def invoke_agent(
    ...
    Bedrock models per (agent, slot). Backend forwards to the AgentCore
    runtime payload so each agent can pick the right BedrockModel.
    """
    ...
    result = await agentcore.invoke_agent(
