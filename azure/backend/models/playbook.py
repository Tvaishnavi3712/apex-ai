...
class TriggerType(str, Enum):
    ...
    GCS = "gcs"            # GCP Cloud Storage (GCS + Pub/Sub notifications)
    AZURE_BLOB = "azure_blob"  # Azure Blob Storage (Event Grid)
...
class DataSource(BaseModel):
    ...
    # S3: bucket, prefix
    # Cosmos DB: container, partition_key
    # API: endpoint, method, headers


class ActionReference(BaseModel):
    """Reference to an action in the playbook"""
    id: Optional[str] = Field(None, description="Action ID within the playbook")
    name: str = Field(..., description="Action name")
    action_id: Optional[str] = Field(None, description="Reference to registered action ID (e.g., aerospace_defense.contract_lookup)")
    handler: Optional[str] = Field(None, description="Handler path (alias for action_id)")
    ...
    required: bool = Field(default=True, description="Whether this action is required")
...
class Playbook(PlaybookBase):
    ...
    agent_id: Optional[str] = Field(None, description="Associated Foundry agent id")
    agent_resource_id: Optional[str] = Field(None, description="Associated Foundry agent resource id")
