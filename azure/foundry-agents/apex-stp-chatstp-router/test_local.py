...
def test_delegation_inproc() -> None:
    ...
    print("[2/3] in-process delegation to all 4 specialists (skipping Azure OpenAI LLM call)")

    # We invoke each delegate_to_* tool with a representative query. Each one
    # ends up calling the specialist's `invoke`, which makes a Azure OpenAI call.
    # Skip the actual Azure OpenAI call here — just verify the routing wires up.
    if os.environ.get("APEX_SKIP_AZURE_OPENAI", "1") == "1":
        print("    [skip] APEX_SKIP_AZURE_OPENAI=1 — delegation wiring only")
...
def test_entrypoint() -> None:
    if os.environ.get("APEX_SKIP_AZURE_OPENAI", "1") == "1":
        print("[3/3] [skip] router entrypoint requires Azure OpenAI")
        return
    print("[3/3] full router roundtrip via Azure OpenAI")
