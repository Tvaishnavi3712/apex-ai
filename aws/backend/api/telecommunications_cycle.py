...
# DynamoDB write at the end of _record_cycle.
...
async def cycle_start(
    ...
    Bucket resolution for Path B (priority order):
      ...
      3. `S3_DOCUMENTS_INCOMING` env var (legacy fallback)
    """
    ...
    if file is not None:
        ...
        try:
            ...
            resolved_bucket = (
                ...
                or request.headers.get("X-Apex-Bucket")
                or os.environ.get("S3_DOCUMENTS_INCOMING", "")
            )
            if not resolved_bucket:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                    detail="No bucket — pass `bucket` form field, "
                                           "X-Apex-Bucket header, or set S3_DOCUMENTS_INCOMING env.")
            tmp_dir = Path("/tmp/apex-cycles")
            tmp_dir.mkdir(parents=True, exist_ok=True)
